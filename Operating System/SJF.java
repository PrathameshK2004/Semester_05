...
public class SJF {
    ...
    public static void main(String args[]) {
        ...
        int temp, start = 0, total = 0;                           //2
        float avgwt = 0, avgTAT = 0;

        for (int i = 0; i < n; i++) {
            System.out.println(" ");
            process[i] = (i + 1);
            System.out.print("Enter Arrival Time for processor P" + (i + 1) + ":");
            arrivaltime[i] = sc.nextInt();
            System.out.print("Enter Burst Time for processor P" + (i + 1) + ": ");
            burstTime[i] = sc.nextInt();
            remburstTime[i] = burstTime[i];                       //3
            visit[i] = 0;
        }
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
